	...
	function getSpecifiedMatch($projectId, $divisionId, $teamId, $matchId)
	{
		if (!$this->_match)
		{
			...
			$query =  ' SELECT m.*, DATE_FORMAT(m.time_present, "%H:%i") time_present, 
				...
				WHERE DATE_ADD(m.match_date, INTERVAL '.$this->_db->Quote($expiry_time).' MINUTE) >= NOW()
				AND m.cancel=0
				AND m.published = 1';
			if ($matchId)
			{
			...
			else
			{
				...
				if ($teamId)
				{
					$query .= ' AND '
						. ' ( '
						. '       t1.id = '. $this->_db->Quote($teamId).' OR '
						. '       t2.id = '. $this->_db->Quote($teamId)
						. ' ) ';
				}
				else
				{
			...
			if ($divisionId)
			{
				$query .= ' AND pt1.division_id = '.$this->_db->Quote($divisionId);
			}
