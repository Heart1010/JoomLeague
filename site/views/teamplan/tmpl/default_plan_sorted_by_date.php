...
?>
    <?php
        ...
        {
            if (($this->config['show_matchday']) && ($MatchDay != $match->name))
            {
                ?>
            ...
            <?php
            if (($this->config['show_date']) && ($MatchDateLine != JoomleagueHelper::getMatchDate($match, JText::_('COM_JOOMLEAGUE_CLUBPLAN_MATCHDATE'))))
            {
            ?>
